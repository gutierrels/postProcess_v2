...
SimConfig parseConfig(const std::string &filename) {
  ...
  while (std::getline(infoS, line)) {
    ...
    if (key == "Counts")
      cfg.nCounts = std::stoull(value);
    else if (key == "DetectorSizeX")
      cfg.header.detectorSizeX = std::stod(value);
    else if (key == "DetectorSizeY")
      cfg.header.detectorSizeY = std::stod(value);
    else if (key == "DistanceBetweenDetectors")
      cfg.header.detectorDistance = std::stod(value);
    else if (key == "ModulesPerRing")
      cfg.header.moduleNumber = std::stoul(value);
    else if (key == "NumberOfRings")
      cfg.header.ringNumber = std::stoul(value);
    else if (key == "DistanceBetweenRings")
      cfg.header.ringDistance = std::stod(value);
    else if (key == "IsotopeName")
      strncpy(cfg.header.isotope, value.c_str(),
              sizeof(cfg.header.isotope) - 1);
    else if (key == "IsotopeHalfLife")
      cfg.header.isotopeHalfLife = std::stod(value);
    else if (key == "EnergyResolution")
      cfg.eRes = std::stod(value) / 100.0;
    else if (key == "PositionResolution")
      cfg.pRes = std::stod(value);
    else if (key == "TimeResolution")
      cfg.tRes = std::stod(value);
    else if (key == "CoincidenceWindow")
      cfg.coincidenceWindow = std::stod(value);
    else if (key == "EnergyWindow")
      cfg.energyWindow = std::stod(value) / 100.0;
    else if (key == "ModuleReconstructionBinsX")
      cfg.nBinsX = std::stoul(value);
    else if (key == "ModuleReconstructionBinsY")
      cfg.nBinsY = std::stoul(value);
    else if (key == "DetectorPixelsX")
      cfg.nBinsNormX = std::stoul(value);
    else if (key == "DetectorPixelsY")
      cfg.nBinsNormY = std::stoul(value);
    else if (key == "CrystalDepth")
      ...
      int auxInt = std::stoi(value);
      if (auxInt ==
          static_cast<int>(CoincidenceMethod::TAKE_WINNER_IF_ALL_ARE_GOODS))
        cfg.coinMethod = CoincidenceMethod::TAKE_WINNER_IF_ALL_ARE_GOODS;
      else if (auxInt == static_cast<int>(CoincidenceMethod::TAKE_CLOSEST))
        cfg.coinMethod = CoincidenceMethod::TAKE_CLOSEST;
      else if (auxInt == static_cast<int>(CoincidenceMethod::TAKE_SAME_HISTORY))
      ...
      else if (auxInt == static_cast<int>(ProjectionMethod::FIT_IN_DETECTOR))
        cfg.projectionMethod = ProjectionMethod::FIT_IN_DETECTOR;
      else
        cfg.projectionMethod = ProjectionMethod::NONE;
    } else if (key == "PairListFilename")
      cfg.pairListFilename = value;
    else if (key == "UseLogicalDetectors")
      cfg.useLogicalDetectors = (std::stoi(value) == 1);
    else if (key == "LogicalRings")
      cfg.nLogicalRings = std::stoul(value);
    else if (key == "DiscardMultiples")
      cfg.discardMultiples = std::stoi(value);
    else if (key == "GenerateHistogram") {
      int auxInt = std::stoi(value);
      if (auxInt == static_cast<int>(GenerateHistogram::LOR_INDEX))
        cfg.generateHistogram = GenerateHistogram::LOR_INDEX;
      else
        cfg.generateHistogram = GenerateHistogram::NONE;
    } else if (key == "SaveWeight")
      cfg.saveWeight = (std::stoi(value) == 1);
    else if (key == "SaveMetadata")
      cfg.saveMetadata = (std::stoi(value) == 1);
    else {
  ...
  if (cfg.nBinsNormX == cfg.nBinsNormY) {
    ...
    throw std::runtime_error("Not square detectors: '" +
                             std::to_string(cfg.nBinsNormX) + "' and '" +
                             std::to_string(cfg.nBinsNormY) + "' pixels");
  }

  // Validate logical detector
