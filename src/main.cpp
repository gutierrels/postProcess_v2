...
int main(int argc, char **argv) {
  try {
    ...
    CoincidenceEngine engine(cfg.coinMethod, &pairs, geo.modPerRing,
                             cfg.useLogicalDetectors, cfg.eminkeV, cfg.emaxKev,
                             cfg.promptGammaMin, cfg.promptGammaMax);
    ...
    while (!reader.allDone()) {
      ...
      double timeMargin =
          (cfg.tRes > 0.0) ? (5.0 * cfg.tRes / constants::FWHM_TO_SIGMA) : 0.0;
      ...
      // In MULTIPLEXING mode, if window[0] is a prompt gamma (Caso 3),
      // search for a valid 511-511 pair in the rest of the window and swap
      // it to the front so the triple is not lost.
      if (cfg.coinMethod == CoincidenceMethod::MULTIPLEXING &&
          window[0].e >= cfg.promptGammaMin &&
          window[0].e <= cfg.promptGammaMax) {
        bool foundSwap = false;
        for (size_t j = 1; j < windowSingles && !foundSwap; ++j) {
          // window[j] must be a 511 keV photon
          if (window[j].e < cfg.eminkeV || window[j].e > cfg.emaxKev)
            continue;
          for (size_t k = j + 1; k < windowSingles && !foundSwap; ++k) {
            // window[k] must also be a 511 keV photon
            if (window[k].e < cfg.eminkeV || window[k].e > cfg.emaxKev)
              continue;
            unsigned modJ =
                cfg.useLogicalDetectors
                    ? window[j].module
                    : sim2devModule(geo.modPerRing, window[j].module);
            unsigned modK =
                cfg.useLogicalDetectors
                    ? window[k].module
                    : sim2devModule(geo.modPerRing, window[k].module);
            if (getPair(pairs, modJ, modK) < pairs.size()) {
              std::swap(window[0], window[j]);
              foundSwap = true;
            }
          }
        }
      ...
      c.time = window[0].t;
      c.amount = 1.0;
      c.gate_flag = result.isTriple ? 1 : 0;
      ...
      if (result.iCoincidence > 0) {
        ...
        if (cfg.projectionMethod == ProjectionMethod::NONE) {
          ...
          double p1Blur[3] = {window[0].x + pBlur(gen),
                              window[0].y + pBlur(gen),
          ...
          p1 = project(p1Blur, p2Blur, geo.detectorSizeX, geo.detectorSizeY,
                       cfg.detectorDepth, geo.ringRad,
                       geo.Rz[window[0].module % geo.modPerRing],
                       geo.Dz[window[0].module / geo.modPerRing],
                       cfg.projectionMethod ==
                           ProjectionMethod::EXTEND_DETECTOR);
          p2 = project(
              p2Blur, p1Blur, geo.detectorSizeX, geo.detectorSizeY,
              cfg.detectorDepth, geo.ringRad,
              geo.Rz[window[result.iCoincidence].module % geo.modPerRing],
              geo.Dz[window[result.iCoincidence].module / geo.modPerRing],
        ...
        if (p1[0] >= geo.detectorSizeX || p1[0] <= 0.0 ||
          ...
          hists.accumulateCoincidence(window[0], window[result.iCoincidence],
                                      p1, p2, c, lorIdx);
        }
      ...
      if (iterations % 100000 == 0) {
        ...
        printf("%sSimultaneous processing singles: %zu\n%s\n",
               reader.progressString().c_str(), window.size(),
               warnings.c_str());
        warnings.clear();
      }
      ++iterations;
