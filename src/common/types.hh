...
enum class CoincidenceMethod : unsigned {
...
enum class GenerateHistogram : unsigned {
  NONE = 0,
  LOR_INDEX,
};
...
struct SingleEvent {
  ...
  inline int readRawSingle(FILE *f, bool saveWeight, bool saveMetadata) {
    ...
    t = constants::SENTINEL_TIME;
    true_t = constants::SENTINEL_TIME;
    return 1;
  }

  inline void applyBlurring(const double eRes, const double tRes,
                            std::normal_distribution<double> &normDist,
                            std::mt19937 &gen) {
    true_t = t;

    // Apply energy blurring
    double sigma = e * eRes / constants::FWHM_TO_SIGMA;
    double normVal = normDist(gen);
    e += sigma * normVal;

    // Apply time blurring
    if (tRes > 0.0) {
      double sigma_t = tRes / constants::FWHM_TO_SIGMA;
  ...
  inline int readPenRed(FILE *f, const unsigned m, const double emin,
    ...
    while (e < emin || e > emax) {
      if (readRawSingle(f, saveWeight, saveMetadata) != 0) {
        return 1;
      }
      applyBlurring(eRes, tRes, normDist, gen);
    }

    // Convert energy to keV
    e /= constants::EV_PER_KEV;
  ...
  inline bool operator>(const SingleEvent &o) const { return t > o.t; }
  inline bool operator<(const SingleEvent &o) const { return t < o.t; }
  inline bool operator==(const SingleEvent &o) const { return t == o.t; }
};
...
struct DetectorPair {
  ...
  DetectorPair() : a(0), b(0) {}
  DetectorPair(const unsigned _a, const unsigned _b) : a(_a), b(_b) {}
  ...
  inline bool operator==(const DetectorPair &o) const {
    return (a == o.a && b == o.b) || (a == o.b && b == o.a);
  }
};

inline unsigned getPair(const std::vector<DetectorPair> &list, const unsigned a,
                        const unsigned b) {

  const DetectorPair p(a, b);
