...
int main(int argc, char** argv){
    ...
    for(size_t i = 0; i < fmods.size(); ++i){
      int err = noInTimeSingles[i].readPenRed(fmods[i], i, emin, emax, eRes, normDist, gen);
      if(err != 0){
    ...
    auto firstSingleIt = std::min_element(noInTimeSingles.begin(), noInTimeSingles.end());
    ...
    int errRead = firstSingleIt->readPenRed(fmods[firstSingleMod], firstSingleMod, emin, emax, eRes, normDist, gen);
    if(errRead != 0){
      if(errRead == 1){
	//End of file
    ...
    const size_t histNormPairSize = nBinsNormX*nBinsNormY*nBinsNormX*nBinsNormY;
    const size_t histNormCentralLOR =
      (nBinsNormX*nBinsNormY + 1)*(nBinsNormX*(nBinsNormY/2) + nBinsNormX/2);
	...
	while(noInTimeSingles[i].t < endTime){
	  ...
	  errRead = noInTimeSingles[i].readPenRed(fmods[i], i, emin, emax, eRes, normDist, gen);
	  if(errRead != 0){
	    if(errRead == 1){
	      //End of file
	      fclose(fmods[i]);
	...
	for(size_t i = 1; i < nextSingles.size(); ++i){
	  // Get pair
	  localPair = getPair(pairIndexes, nextSingDevMod, sim2devModule(modPerRing,nextSingles[i].module));
	...
	if(projectionMethod == PROJECTION_METHOD::NONE){
	  ...
	  lor0 = std::array<double, 3>{
	    nextSingles[0].x,
	    nextSingles[0].y,
	    nextSingles[0].z};
	  lor1 = std::array<double, 3>{
	    nextSingles[iCoincidence].x,
	    nextSingles[iCoincidence].y,
	    nextSingles[iCoincidence].z
	  };
	  
	  double p1Aux[3] = {nextSingles[0].x, nextSingles[0].y, nextSingles[0].z};
	  p1 = toLocal(p1Aux,
	  ...
	  double p2Aux[3] =
	    {nextSingles[iCoincidence].x, nextSingles[iCoincidence].y, nextSingles[iCoincidence].z};
	  p2 = toLocal(p2Aux,
		       detectorSizeX, detectorSizeY,
		       detectorDepth, ringRad,
	...
	//Check bounds
	if(p1[0] >=  detectorSizeX  || p1[0] <= 0.0 ||
	   p1[1] >=  detectorSizeY  || p1[1] <= 0.0 ||
	   p1[2] >=  detectorDepth  || p1[2] < -1.0e-3 ||
	   p2[0] >=  detectorSizeX  || p2[0] <= 0.0 ||
	   p2[1] >=  detectorSizeY  || p2[1] <= 0.0 ||
	  ...
	  if(projectionMethod != PROJECTION_METHOD::EXTEND_DETECTOR){
	    ...
	    snprintf(auxBuffer, 1000,
		     ...
		     nextSingles[0].z,
		     nextSingles[0].t,
		     nextSingles[iCoincidence].e,
		     nextSingles[iCoincidence].x,
		     nextSingles[iCoincidence].y,
		     nextSingles[iCoincidence].z,
		     nextSingles[iCoincidence].t,
		     nextSingles[0].e,
		     p1[0], p1[1], p1[2],
		     nextSingles[0].t,
		     nextSingles[iCoincidence].e,
	  ...
	  if(outputFormat == OUTPUT_FORMAT::CASTOR){
	    ...
	    std::pair<uint32_t, uint32_t> lor = castorLOR(nextSingles[0].module,
							  nextSingles[iCoincidence].module,
							  p1, p2,
							  modPerRing,
	    ...
	    if(pairIndexes[iPair].a != sim2devModule(modPerRing, nextSingles[0].module)){
	    ...
	    if(attenuationMethod == ATTENUATION_METHOD::CYLINDER){
	      float attenuationFactor = attenuationFactorWithAir(lor0,
								 lor1,
								 3.29841E-02,
								 2.96526E-02*1.20479000E-03,
								 {0.0,0.0,7.5},
								 3.5,
								 5.0);
	    ...
	    const unsigned binFirstIndex  = (normBin1Y*nBinsNormX + normBin1X)*nBinsNormX*nBinsNormY;
	    histNorm[pairFirstIndex + binFirstIndex + normBin2Y*nBinsNormX + normBin2X] += 1.0;
	  ...
	  if(ebin >= 0 && ebin < nBinsE){
	    ...
	    if(zbin >= 0 && zbin < nBinsZ){
	      ++histMez[nextSingles[0].module*nBinsE*nBinsZ + ebin*nBinsZ + zbin];
	    }
	  }
	  ...
	  if(ebin >= 0 && ebin < nBinsE){
	    ...
	    if(zbin >= 0 && zbin < nBinsZ){
	      ++histMez[nextSingles[iCoincidence].module*nBinsE*nBinsZ + ebin*nBinsZ + zbin];
	    }
	  }
	}
	...
	nextSingles.erase(nextSingles.begin());
      ...
      //Save bruker format
      if(outputFormat != OUTPUT_FORMAT::CASTOR){
	if(!outputFormat == OUTPUT_FORMAT::BRUKER_LM_ONLY_COINCIDENCES || acceptedCoincidence)
	...
	//Get the first single
	firstSingleIt = std::min_element(noInTimeSingles.begin(), noInTimeSingles.end());
	...
	if(fmods[firstSingleMod] != nullptr){
	  errRead = firstSingleIt->readPenRed(fmods[firstSingleMod], firstSingleMod, emin, emax, eRes, normDist, gen);
	  if(errRead != 0){
	    if(errRead == 1){
	      //End of file
	      fclose(fmods[firstSingleMod]);
	...
	progressSstring += "Simultaneous processing singles: " + std::to_string(nextSingles.size()) + "\n";
	...
	fprintf(fout,
		...
		maxTimestamp,
		static_cast<unsigned long>(cHistNorm.size()));
	...
	fprintf(fcH,
		...
		maxTimestamp,
		static_cast<unsigned long>(nCoincidences),
	...
	for(size_t i = 0; i < nBinsE; ++i){
	  ...
	  for(size_t j = 0; j < nBinsZ; ++j){
	    fprintf(fout, "%u ", histMez[imod*nBinsE*nBinsZ + i*nBinsZ + j]);
	  }
	  fprintf(fout,"\n");
	}
	fclose(fout);
